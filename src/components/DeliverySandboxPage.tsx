...
import {
  ...
  Truck, CreditCard, ShoppingBag, User, MapPin,
  ChevronRight, ChevronLeft, Package, Minus, Plus,
} from 'lucide-react';
...
export function DeliverySandboxPage() {
  ...
  const [deliveryFee, setDeliveryFee] = useState<{ cost: number; breakdown?: { baseFare: number; distanceFee: number; timeFee: number; distanceKm: number; estimatedMinutes: number } } | null>(null);
  ...
  const reset = () => {
    ...
    tokenRef.current = '';
    setStep(0); setDir(1); setLoading(false); setError('');
    setToken(''); setCustomerName(''); setCart(new Map());
    setOrder(null); setPayRef(''); setPayUrl('');
    setTrackingLink(''); setJobId(''); setDeliveryStatus(''); setPollCount(0);
  };

  // Fetch products when reaching step 1 — vendor name comes from populated vendor field
  useEffect(() => {
    if (step !== 1 || products.length > 0) return;
    call('GET', '/products?limit=8').then(d => {
      ...
      list.forEach((p, i) => {
        ...
        if (!names[vid]) {
          ...
          names[vid] = v?.vendorInfo?.businessName ?? v?.firstName ?? `Vendor ${String.fromCharCode(65 + i)}`;
        }
      });
  ...
  useEffect(() => {
    ...
    setFeeLoading(true);
    const params = new URLSearchParams({ state: shipping.state, city: shipping.city, vendors: String(vendorCount) });
    call('GET', `/deliveries/cost?${params}`).then(d => {
  ...
  const handleInitPay = async () => {
    ...
    setLoading(true); setError('');
    try {
      const d = await call('POST', '/payments/initialize', tokenRef.current, { orderId: order._id });
            ...
            <React.Fragment key={s.id}>
              <div className="flex flex-col items-center gap-1 flex-shrink-0">
                <div className={cn(
                  ...
                  active ? 'bg-ebunly-orange text-white shadow-lg shadow-ebunly-orange/30' :
      ...
      <div className="overflow-hidden relative min-h-[480px]">
        <AnimatePresence mode="wait" custom={dir}>
          ...
          >
              ...
              >
                <div className="space-y-4">
                  ...
                  <p className="text-xs text-gray-400">
                    Pre-filled with the test account. Change it if you want to test with a different customer.
                  </p>
                  {error && <ErrorBox message={error} />}
                  <Button onClick={handleLogin} disabled={loading} className="w-full bg-ebunly-orange hover:bg-ebunly-orange-dark text-white h-11">
            ...
            {step === 1 && (() => {
              const cartTotal = [...cart.values()].reduce((s, e) => s + e.product.basePrice * e.qty, 0);
              const vendorIds = [...new Set([...cart.values()].map(e => vendorId(e.product)))];
              const toggleProduct = (p: Product) => {
                setCart(prev => {
                  const next = new Map(prev);
                  if (next.has(p._id)) { next.delete(p._id); } else { next.set(p._id, { product: p, qty: 1 }); }
                  return next;
                });
              };
              const setCartQty = (id: string, delta: number) => {
                setCart(prev => {
                  ...
                  if (newQty < 1) { next.delete(id); } else { next.set(id, { ...entry, qty: newQty }); }
                  return next;
                });
              };
              return (
                        ...
                        return (
                          <div
                            ...
                            className={cn(
                              ...
                              inCart ? 'border-ebunly-orange bg-orange-50 shadow-md shadow-ebunly-orange/10' : 'border-gray-200 bg-white'
                            )}
                          >
                            <button className="w-full text-left" onClick={() => toggleProduct(p)}>
                              ...
                              <p className="text-[10px] font-bold text-ebunly-orange/80 uppercase tracking-wide mb-0.5">{vName}</p>
                              <p className="text-xs font-semibold text-gray-800 line-clamp-2">{p.name}</p>
                              <p className="text-sm font-bold text-ebunly-orange mt-1">₦{p.basePrice.toLocaleString()}</p>
                            </button>
                            {inCart && (
                              <div className="flex items-center gap-2 mt-2 pt-2 border-t border-ebunly-orange/20">
                                <button onClick={() => setCartQty(p._id, -1)} className="w-6 h-6 rounded-full border border-ebunly-orange/30 flex items-center justify-center hover:bg-orange-100">
                                  <Minus className="w-3 h-3 text-ebunly-orange" />
                                </button>
                                <span className="flex-1 text-center text-sm font-bold text-ebunly-orange">{entry.qty}</span>
                                <button onClick={() => setCartQty(p._id, 1)} className="w-6 h-6 rounded-full border border-ebunly-orange/30 flex items-center justify-center hover:bg-orange-100">
                  ...
                  {cart.size > 0 && (
                    <div className="mt-4 p-3 bg-gray-50 rounded-xl space-y-1.5">
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-500">{cart.size} item{cart.size > 1 ? 's' : ''}</span>
                        <span className="font-bold text-gray-900">₦{cartTotal.toLocaleString()}</span>
                      </div>
                      <div className="flex items-center gap-1.5">
                        {vendorIds.map((vid, i) => (
                          <span key={vid} className="px-2 py-0.5 text-[10px] font-bold bg-indigo-100 text-indigo-700 rounded-full">
                            {vendorNames[vid] ?? `Vendor ${String.fromCharCode(65 + i)}`}
                          </span>
                        ))}
                        {vendorIds.length > 1 && (
                          <span className="text-[10px] text-gray-400 ml-1">→ {vendorIds.length} Kwik pickups, 1 rider</span>
                        )}
                      </div>
                    </div>
                  )}
              ...
              >
                <div className="space-y-3">
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div className="col-span-2">
                      <Field label="Street Address">
                        <Input value={shipping.street} onChange={e => setShipping(s => ({ ...s, street: e.target.value }))} />
                      </Field>
                    </div>
                    <Field label="City">
                      ...
                      <div className="py-3 border-t border-gray-100 mt-2 space-y-1.5">
                        {entries.map(({ product, qty }) => (
                          <div key={product._id} className="flex justify-between text-xs text-gray-600">
                            <span className="truncate max-w-[60%]">{product.name} × {qty}</span>
                            <span className="font-medium">₦{(product.basePrice * qty).toLocaleString()}</span>
                          </div>
                        ))}
                          ...
                          <span className="flex items-center gap-1">
                            ...
                            {deliveryFee?.breakdown && (
                              <span className="text-[10px] text-gray-400">
                                {deliveryFee.breakdown.distanceKm}km ~{deliveryFee.breakdown.estimatedMinutes}min · ₦{deliveryFee.breakdown.baseFare}+₦{deliveryFee.breakdown.distanceFee}+₦{deliveryFee.breakdown.timeFee}
                              </span>
                            )}
                          </span>
                          <span className="flex items-center gap-1">
                            {feeLoading ? <Loader2 className="w-3 h-3 animate-spin" /> : `₦${(deliveryFee?.cost ?? 1500).toLocaleString()}`}
                          </span>
                        </div>
                        <div className="flex justify-between text-sm font-bold text-gray-900">
                          <span>Total</span>
                          <span>₦{(itemsTotal + (deliveryFee?.cost ?? 1500)).toLocaleString()}</span>
                        </div>
                      </div>
                    );
                  })()}
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      {loading ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : null}
                      Place Order ({cart.size} item{cart.size !== 1 ? 's' : ''}) <ChevronRight className="w-4 h-4 ml-1" />
                    </Button>
                  </div>
                </div>
              </StepShell>
                  ...
                  <div className="flex items-center gap-3 p-4 bg-green-50 border border-green-200 rounded-xl">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-green-600 mt-0.5">
                        Kwik job ID: <code className="bg-green-100 px-1.5 py-0.5 rounded font-mono">{jobId || '—'}</code>
                      </p>
                    </div>
                  </div>
